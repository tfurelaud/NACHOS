...
void SynchConsoleTest (const char *in, const char *out){
  ...
  /* char s = test_synchconsole->SynchGetChar();
  test_synchconsole->SynchPutChar(s);
  if( s == 'a'){
    test_synchconsole->SynchPutChar('T');
    }*/
  test_synchconsole->SynchPutString("Bonjour\n");

   char s2[16];
   char s3[20];
    test_synchconsole->SynchGetString(s2,16);
    test_synchconsole->SynchPutString(s2);
    test_synchconsole->SynchPutChar('\n');
    test_synchconsole->SynchGetString(s3,20);
    test_synchconsole->SynchPutString(s3);
  ...
  /*char c1[] = "cbcbc";
    test_synchconsole->SynchPutString(c1);*/
